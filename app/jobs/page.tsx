...
export default function JobsPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950">
      <div className="bg-primary-600 dark:bg-primary-700 text-white py-12 sm:py-16">
        <div className="container mx-auto px-4">
          <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-3 sm:mb-4">Find Your Next Mission</h1>
          <p className="text-lg sm:text-xl opacity-90">Discover opportunities that match your clearance and expertise</p>
        </div>
      </div>

      <div className="container mx-auto px-4 -mt-8">
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg dark:shadow-gray-900/50 p-4 sm:p-6 mb-6 sm:mb-8">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <select
              className="px-4 py-3 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
          ...
          <div className="flex items-center gap-4 mt-4 text-sm text-gray-600 dark:text-gray-400">
            <button className="flex items-center gap-2 hover:text-primary-600 dark:hover:text-primary-400">
        ...
        <div className="space-y-4">
          {filteredJobs.map((job) => (
            <div key={job.id} className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 hover:shadow-md dark:hover:shadow-gray-700/50 transition p-4 sm:p-6">
              <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                <div className="flex-1">
                  <div className="flex items-start gap-3 mb-2">
                    <div className="bg-gray-100 dark:bg-gray-700 p-3 rounded-lg">
                      <Briefcase className="h-5 w-5 sm:h-6 sm:w-6 text-gray-600 dark:text-gray-400" />
                    </div>
                    <div className="flex-1">
                      <h3 className="text-lg sm:text-xl font-semibold text-gray-900 dark:text-gray-100">{job.title}</h3>
                  ...
                  <p className="text-sm sm:text-base text-gray-600 dark:text-gray-400 mb-3 sm:mb-4">{job.description}</p>

                  <div className="flex flex-wrap gap-2 mb-4">
                    {job.skills.map((skill, index) => (
                      <span key={index} className="px-2 sm:px-3 py-1 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 text-xs sm:text-sm rounded-full">
                        {skill}
                      </span>
                    ))}
                  </div>
                ...
                <div className="flex flex-col items-end gap-2">
                  {job.matchScore && (
                    <div className="text-right mb-2">
                      <p className="text-xl sm:text-2xl font-bold text-primary-600 dark:text-primary-400">{job.matchScore}%</p>
                      <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-400">match</p>
                    </div>
                  )}
                  <button className="bg-primary-600 text-white px-6 py-2 rounded-lg hover:bg-primary-700 transition whitespace-nowrap">
                    One-Click Apply
                  </button>
                  <button className="text-primary-600 dark:text-primary-400 hover:text-primary-700 dark:hover:text-primary-300 text-xs sm:text-sm">
                    Save Job
                  </button>
                </div>
              </div>
        ...
        {filteredJobs.length === 0 && (
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-8 sm:p-12 text-center">
            <p className="text-gray-600 dark:text-gray-400">No jobs found matching your criteria. Try adjusting your filters.</p>
          </div>
        )}
      </div>
    </div>
